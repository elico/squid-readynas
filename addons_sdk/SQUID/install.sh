...
if grep -q ${name} $ADDON_HOME/addons.conf; then
  orig_vers=`awk -F'!!' '/SQUID/ { print $3 }' $ADDON_HOME/addons.conf | cut -f1 -d'.'`
fi
...
grep -v SQUID_ALLOWED_HOSTS /etc/default/services > /tmp/services
echo "SQUID_ALLOWED_HOSTS=" >> /tmp/services
cp /tmp/services /etc/default/services
rm -f /tmp/services

mkdir -p /c/squid/cache || bye "ERROR: Could not create cache directory.";
chown nobody /c/squid/cache || bye "ERROR: Could not set ownership on cache.";
mkdir -p /c/squid/logs || bye "ERROR: Could not create log directory.";
chown nobody /c/squid/logs || bye "ERROR: Could not set ownership on logs directory.";
sed "s#ALLOWED_HOSTS#127.0.0.1#" /opt/squid/etc/squid.conf.tmpl > /opt/squid/etc/squid.conf
/opt/squid/sbin/squid -z 2>/dev/null || bye "ERROR: Could not initialize cache.";
